...
def make_rotation_batch(rx, ry, rz, order="xyz"):
    Rx = rotx_batch(rx)
    Ry = roty_batch(ry)
    Rz = rotz_batch(rz)
    if order == "xyz":
...
def rotx_batch(theta):
    N = theta.shape[0]
    rotx = torch.zeros([N,3,3])
    rotx[:,0,0] = 1
    rotx[:,1,1] = torch.cos(theta)
    rotx[:,1,2] = -torch.sin(theta)
    rotx[:,2,1] = torch.sin(theta)
    rotx[:,2,2] = torch.cos(theta)
    return rotx
...
def roty_batch(theta):
    N = theta.shape[0]
    roty = torch.zeros([N,3,3])
    roty[:,0,0] = torch.cos(theta)
    roty[:,0,2] = torch.sin(theta)
    roty[:,1,1] = 1
    roty[:,2,0] = -torch.sin(theta)
    roty[:,2,2] = torch.cos(theta)
    return roty
...
def rotz_batch(theta):
    N = theta.shape[0]
    rotz = torch.zeros([N,3,3])
    rotz[:,0,0] = torch.cos(theta)
    rotz[:,0,2] = torch.sin(theta)
    rotz[:,1,1] = 1
    rotz[:,2,0] = -torch.sin(theta)
    rotz[:,2,2] = torch.cos(theta)
    return rotz

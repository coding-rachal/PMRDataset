...
def get_seq_static_lookat_points(seq_name=None, bounds=None):
    # top_source = torch.tensor([ 2.5973, -2.2790, -8.6292])
    # top_target = torch.tensor([ 2.5973, -0.2790,  0.0947])

    # side_source = torch.tensor([ 7.4438, -0.7790, -6.6905])
    # side_target = torch.tensor([ 2.5973, -0.2790,  0.0947])
...
#        return (top_source, top_target), (side_source, side_target)
    ...
    top_source = torch.tensor([0.0, -4.0, -6.0])

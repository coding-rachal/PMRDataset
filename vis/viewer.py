...
def init_viewer(
    img_size, intrins, vis_scale=1.0, bg_paths=None, bg_paths_oppo=None, fps=30,
):
    ...
    # if bg_paths_oppo is not None:
    #     bg_imgs_oppo = [
    #         read_image(p, scale=vis_scale).astype(np.float32) / 255 for p in bg_paths_oppo
    #     ]
    #     vis.set_bg_seq_oppo(bg_imgs_oppo)
    print("VIS", vis)
    return vis
...
class AnimationBase(object):
    ...
    def __init__(self, img_size, intrins=None):
        ...
        self.cam_marker_node_oppo = None
        self.cam_marker_poses_oppo = []
    ...
    def set_camera_seq(self, poses, oppo=False):
        """
        :param poses (*, 4, 4)
        """
    ...
    def add_camera_markers_oppo(self, poses):
        """
        Add a single camera marker node that we move around when we animate
        """
        print("ADDING CAMERA MARKERS")
        if self.cam_marker_node_oppo is None:
            cam_marker = make_camera_marker(up="y")
            self.cam_marker_node_oppo = self.scene.add(
                pyrender.Mesh.from_trimesh(cam_marker, smooth=False)
            )
        poses = transform_pyrender(poses)
        poses = np.asarray(poses)
        self.cam_marker_poses_oppo = poses
    ...
    def update_camera_oppo(self, cam_pose):
        self.acquire_lock()
        self.scene.set_pose(self.cam_node_oppo, pose=cam_pose)
        self.release_lock()
    ...
    def update_frame(self):
        ...
        if self.anim_cameras is not None:
            ...
            if self.cam_marker_node is not None and t < len(self.cam_marker_poses):
                ...
                self.release_lock()
            if self.cam_marker_node_oppo is not None and t < len(self.cam_marker_poses_oppo):
                self.acquire_lock()
                self.scene.set_pose(self.cam_marker_node_oppo, self.cam_marker_poses_oppo[t])
                self.release_lock()

        # # update camera
        # if self.anim_cameras_oppo is not None:
        #     cam_t = min(t, len(self.anim_cameras_oppo) - 1)
        #     cam_pose = self.anim_cameras_oppo[cam_t]
        #     self.update_camera_oppo(cam_pose)
        #     if self.cam_marker_node_oppo is not None and t < len(self.cam_marker_poses_oppo):
        #         self.acquire_lock()
        #         self.scene.set_pose(self.cam_marker_node_oppo, self.cam_marker_poses_oppo[t])
        #         self.release_lock()
...
class OffscreenAnimation(AnimationBase):
    ...
    def set_bg_seq_oppo(self, bg_imgs):
        assert isinstance(bg_imgs, list)
        print(f"setting length {len(bg_imgs)} bg_oppo sequence")
        self.bg_seq_oppo = bg_imgs
    ...
    def render(
        self, render_bg=True, render_bg_oppo=False, render_ground=True, render_cam=True, fac=1.0, **kwargs
    ):
        # render_ground = True
        ...
        # rgba = (np.ones((*self.viewport_size, 3))*255).astype(np.uint8)
        rgba = (np.ones((*self.viewport_size[::-1], 3))*255).astype(np.uint8)
        ...
        img = rgba
        if (render_bg and len(self.bg_seq)) or (render_bg_oppo and len(self.bg_seq_oppo)) > 0:
            H, W = rgba.shape[:2]
            invis_mask = np.ones((H, W, 3))
            mask = np.where(rgba==255, 1, 0)
            invis = mask[:,:,0]*mask[:,:,1]*mask[:,:,2]
            invis_mask[:,:,0] = invis
            invis_mask[:,:,1] = invis
            invis_mask[:,:,2] = invis
            alpha = (1- invis_mask)*fac
            bg_seq = self.bg_seq if render_bg else self.bg_seq_oppo
            t = min(self.anim_idx, len(bg_seq) - 1)
            bg = bg_seq[t]
            rgba = rgba.astype(np.float32) / 255
            # alpha = fac * rgba[..., 3:]
            # alpha = fac*1
            # alpha=alpha*0.7
            alpha_rate = np.sum(alpha)/(alpha.shape[0]*alpha.shape[1]*alpha.shape[2])
    ...
    def render_frames(self, **kwargs):
        ...
        frames = []
        min_fac = 0.3
        step_fac = (1-min_fac)/self.anim_len
        now_fac = min_fac
        for t in range(self.anim_len):
            self.anim_idx = t
            self.update_frame()
    ...
    def render_layers(self, save_dir, composite=False, save_frames=False, fac=0.4):
        ...
        bg_img = self.render(render_bg=False, render_ground=True)
        ...
        if composite:
            rgba = composite_layers(frames, bg_img/255, fac=fac)
            imageio.imwrite(f"{save_dir}/composite.png", (255 * rgba).astype(np.uint8))


# def composite_layers(frames, bg_img, fac=0.5):
...
def composite_layers(frames, bg_img, fac=0.5):
    ...
    skip = 5
    ind = -1
    step_fac = (1-min_fac)/len(frames)*skip
    comp = bg_img[..., :3]
    for frame in frames:
        ind += 1
        if ind%skip!=0:
            continue
        mask = np.where(frame==255, 1, 0)
        invis = mask[:,:,0]*mask[:,:,1]*mask[:,:,2]
        invis_mask[:,:,0] = invis
        invis_mask[:,:,1] = invis
        invis_mask[:,:,2] = invis
        ...
        alpha = (1 - invis_mask)
        # # alpha = fac * frame[..., 3:]
        # alpha = (1 - invis_mask)*now_fac
        # now_fac+=step_fac
        # now_fac = min(1.0, now_fac)
        frame = frame[:,:,:3]
        comp = alpha*frame + (1-alpha)*comp
        total_invis *= invis_mask
        totalinvis_rate = np.sum(total_invis)/(total_invis.shape[0]*total_invis.shape[1]*total_invis.shape[2])
        print(now_fac, totalinvis_rate)
        # comp += vis * alpha * frame[..., :3]
        # vis *= 1 - alpha

   
    # comp = total_invis * bg_img[..., :3] + (1 - total_invis)* comp
    totalinvis_rate = np.sum(total_invis)/(total_invis.shape[0]*total_invis.shape[1]*total_invis.shape[2])
